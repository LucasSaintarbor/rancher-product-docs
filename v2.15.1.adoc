...
:release-version: v2.15.1
:rn-component-version: v2.15
:previous-release-version: v2.15.0
...
ifeval::["{build-type}" != "community"]
Rancher {release-version} is the latest patch release of Rancher. This is a Prime version release that introduces new features, enhancements, and bug fixes. To learn more about Rancher Prime, see our page on the https://www.rancher.com/products/rancher-platform[Rancher Prime Platform].
endif::[]
...
** Rancher v2.15.1 adds support for **Kubernetes v1.36** while removing support for Kubernetes v1.33.
...
* A known issue exists with Rancher v2.15.1 and Kubernetes v1.36 clusters where **Rancher Backups** is not listed under the **Cluster Tools** due to a Backups chart issue. See https://github.com/rancher/backup-restore-operator/pull/1055[#1055].
...
[IMPORTANT]
====
Starting in Rancher v2.15.1, new monitoring installations use the decoupled `rancher-monitoring-dashboards` chart.
...
* **Removal**: Support for Ember-based UI plugins for cluster and node drivers (deprecated since Rancher v2.11.0) is removed in Rancher v2.15.1. Migrate your plugins to the https://extensions.rancher.io/[UI Extensions Framework]. For details, see https://github.com/rancher/dashboard/issues/14005[#14005].
...
* Rancher v2.15.1 adds support for Cluster API (CAPI) v1.13.2. See https://github.com/rancher/rancher/issues/54304[#54304].
...
* Rancher now supports mixed-architecture (x86_64 and ARM64) Linux deployments for both the Rancher Management Server and downstream clusters.

[#_known_issues_cluster_provisioning]
=== Known Issues

* Google Cloud has deprecated the "No channel" option for GKE clusters as of August 21, 2026. All clusters must now https://docs.cloud.google.com/kubernetes-engine/docs/concepts/release-channels#channels[use a release channel]. Currently, GCP's automatic out-of-band upgrades can cause state drift in Rancher, which may block cluster edits, break node pool updates, or cause provisioning failures. An upcoming out-of-band GKE chart release will prevent these issues by automatically assigning new clusters to a release channel by default. See https://github.com/rancher/rancher/issues/56879[#56879].
...
[#_features_and_enhancements_continuous_delivery_fleet]
=== Features and Enhancements

* SUSE Continuous Delivery introduces native support for the SUSE Application Collection as a dedicated deployment flow under App Bundles. Users can authenticate using their apps.rancher.io access credentials to browse, configure, and deploy curated application charts directly to downstream clusters, with Fleet automatically handling imagePullSecrets management for target workloads. See https://github.com/rancher/dashboard/issues/14627[#14627].
...
//Community v2.15.1 behavior changes, remove conditional in following release
ifeval::["{build-type}" == "community"]
...
//Community v2.15.1 known issue, remove conditional in following release
ifeval::["{build-type}" == "community"]
[#_long_standing_known_issues_rancher_general]
=== Long-standing Known Issues - Rancher General

* *Rancher v2.15.0:*
* A known issue exists with Rancher v2.15.0 and Kubernetes v1.36 clusters where **Rancher Backups** is not listed under the **Cluster Tools** due to a Backups chart issue. See https://github.com/rancher/backup-restore-operator/pull/1055[#1055].
endif::[]
