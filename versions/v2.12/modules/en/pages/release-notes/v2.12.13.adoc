...
:release-version: v2.12.13
:rn-component-version: v2.12
:previous-release-version: v2.12.12
...
:rn-ui-server-pagination: xref:rancher-admin/global-configuration/ui-server-side-pagination.adoc
:rn-ui-server-pagination-known-issues: xref:rancher-admin/global-configuration/ui-server-side-pagination.adoc#_known_limitations_of_ui_server_side_pagination
...
* Fixed a security vulnerability in Rancher's unauthenticated cluster import endpoint where an attacker could enumerate valid cluster IDs and detect private registry usage by observing distinguishable HTTP response codes. The fix validates the request token before any cluster object is loaded, ensuring all unauthenticated requests receive a uniform response. For more information, refer to https://github.com/rancher/rancher/security/advisories/GHSA-23h9-rr79-r3gh[CVE-2026-55998].
* Fixed a security vulnerability where an unauthenticated attacker with cluster-internal network access could flood TLS listeners in `cattle-cluster-agent` and in the Rancher server with requests containing arbitrary hostnames, causing serving certificates to accumulate excessive Subject Alternative Names (SANs) until TLS handshakes failed. The fix applies CN filtering to reject attacker-supplied SNI hostnames and caps the maximum number of SANs per certificate. For more information, refer to https://github.com/rancher/rancher/security/advisories/GHSA-9jxv-832x-45q9[CVE-2026-55996].
* Fixed a critical security vulnerability in Rancher's impersonation middleware (`pkg/auth/requests/impersonate.go`) where an authenticated user with the default `user` global role could exploit a cross-cluster impersonation flaw to impersonate privileged identities, including `system:masters`, against the local Rancher management cluster. This allowed full administrative takeover of the Rancher control plane and all managed downstream clusters and secrets. For more information, refer to https://github.com/rancher/rancher/security/advisories/GHSA-v584-7w32-jwpq[CVE-2026-44945].
* Rancher now protects against Denial of Service attacks on login endpoints by only reading request bodies up to 1MiB. Users with admin permissions can alter this limit by changing the setting `public-api-body-limit` by updating the corresponding global setting resource through the v3 API or via kubectl. For more information, refer to https://github.com/rancher/rancher/security/advisories/GHSA-g4f6-44g4-23xm[CVE-2026-59675].
...
[#_known_issues_rancher_app_global_ui]
=== Known Issues
...
[#_features_and_enhancements_rancher_webhook]
=== Features and Enhancements

[#_major_bug_fixes_rancher_webhook]
=== Major Bug Fixes

[#_known_issues_rancher_webhook]
=== Known Issues

[#_k3s_provisioning]
== K3s Provisioning

[#_features_and_enhancements_k3s_provisioning]
=== Features and Enhancements

[#_major_bug_fixes_k3s_provisioning]
=== Major Bug Fixes

[#_known_issues_k3s_provisioning]
=== Known Issues

[#_rke2_provisioning]
== RKE2 Provisioning

[#_features_and_enhancements_rke2_provisioning]
=== Features and Enhancements

[#_major_bug_fixes_rke2_provisioning]
=== Major Bug Fixes

[#_known_issues_rke2_provisioning]
=== Known Issues

[#_backup_restore]
== Backup/Restore

[#_features_and_enhancements_backup_restore]
=== Features and Enhancements

[#_major_bug_fixes_backup_restore]
=== Major Bug Fixes

[#_known_issues_backup_restore]
=== Known Issues
...
[#_major_bug_fixes_continuous_delivery_fleet]
=== Major Bug Fixes

[#_known_issues_continuous_delivery_fleet]
=== Known Issues

[#_suse_virtualization_harvester]
== SUSE Virtualization (Harvester)

[#_features_and_enhancements_suse_virtualization_harvester]
=== Features and Enhancements

[#_major_bug_fixes_suse_virtualization_harvester]
=== Major Bug Fixes

[#_known_issues_suse_virtualization_harvester]
=== Known Issues
...
** The feature flag `ui-sql-cache` (Server-Side Pagination) is now enabled by default in Rancher. Please refer to the {rn-ui-server-pagination}[UI Server-Side Pagination] document for more information.
+
...
+
This update has introduced limitations which are outlined in the {rn-ui-server-pagination-known-issues}[UI Server Side Pagination documentation]. See https://github.com/rancher/rancher/issues/48691[#48691] and https://github.com/rancher/dashboard/issues/12975[#12975] for more information.
